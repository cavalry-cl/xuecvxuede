...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        ...
        src_mask = src_mask.unsqueeze(1).expand(-1, tgt_len, -1)

        causal_mask = torch.tril(torch.ones((tgt_len, tgt_len), dtype=torch.bool))

        tgt_mask = causal_mask & (tgt_mask.unsqueeze(1).expand(-1, tgt_len, -1))

        return self.decoder(self.tgt_embed(tgt), memory, src_mask, tgt_mask)

    def forward(self, src, tgt, src_mask, tgt_mask):
        # encoder的结果作为decoder的memory参数传入，进行decode
        encoder_res = self.encoder(input_ids=src, attention_mask=src_mask)["last_hidden_state"]
        decoder_res = self.decode(encoder_res, src_mask, tgt, tgt_mask)
        tgt_emb_res = self.generator(decoder_res)
        return tgt_emb_res
...
def make_model(encoder, tgt_vocab, N=6, d_model=768, d_ff=2048, h=8, dropout=0.1):
    c = copy.deepcopy
    # 实例化Attention对象
